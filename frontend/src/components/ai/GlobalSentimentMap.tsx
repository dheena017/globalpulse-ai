    ...
    <GlassCard glowColor="cyan" className="space-y-6 bg-white border border-slate-200/90 shadow-md">
      <div className="flex items-center justify-between border-b border-slate-100 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-bold text-slate-900 font-heading">Global Geopolitical Mood & Sentiment Radar</h3>
            <p className="text-xs text-slate-500">Real-time tone & optimism barometer across world continents</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {regions.map((reg) => (
          <div
            key={reg.code}
            className="rounded-2xl border border-slate-200 bg-slate-50/50 p-4 transition-all hover:border-sky-300 hover:bg-white hover:shadow-sm"
          >
            ...
            <p className="text-xs font-semibold text-emerald-700 mb-1">{reg.sentiment_summary}</p>
            <p className="text-[11px] text-slate-600 line-clamp-2 leading-relaxed">{reg.description}</p>
          </div>
        ))}
      </div>
    </GlassCard>
